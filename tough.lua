...
local Library = {
	...
	Accent = Color3.fromRGB(130, 103, 211),
...
do	-- // Library Functions
	...
	function Library:UpdateNotificationTray(position)
		for i, v in pairs(Library.Notifications) do 
			local Position = position == "Middle" and Vector2.new(ViewportSize.X/2 - (v["Objects"][3].TextBounds.X + 4)/2,600) or Vector2.new(20, 20)
	...
	function Library:Notification(message, duration, color, position)
		...
		local Container = Instance.new('Frame', Library.ScreenGui)
		...
		local Accent = Instance.new('Frame', Background)
		local Progress = Instance.new('Frame', Background)

		local Position = position == "Middle" and Vector2.new(ViewportSize.X/2 - (TextLabel.TextBounds.X + 4)/2, 600) or Vector2.new(20, 20)

		Container.Name = "NotifContainer"
		Container.Position = UDim2.new(0,Position.X, 0, Position.Y)
		Container.Size = UDim2.new(0,TextLabel.TextBounds.X + 4,0,20)
		...
		function notification:remove()
			...
			Library:UpdateNotificationTray(position)
			notification.Container:Destroy()
		end
	
		task.spawn(function()
			Background.AnchorPoint = Vector2.new(1,0)
			TweenService:Create(Progress, TweenInfo.new(duration or 5, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {BackgroundColor3 = Color3.new(0,1,0)}):Play()
			task.wait(duration)
			TweenService:Create(Background, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {AnchorPoint = Vector2.new(1,0)}):Play()
			for i,v in next, notification.Objects do
				TweenService:Create(v, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
			end
			TweenService:Create(TextLabel, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 1}):Play()
			TweenService:Create(UIStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Transparency = 1}):Play()
		end)
		...
		Container.Position = UDim2.new(0,Position.X,0,Position.Y + (table.find(Library.Notifications, notification) * 25))
		Container.Size = UDim2.new(0,TextLabel.TextBounds.X + 4,0,18)
		Library:UpdateNotificationTray(position)
	...
	function Library.NextFlag()
		Library.UnamedFlags = Library.UnamedFlags + 1
		return string.format("%.14g", Library.UnamedFlags)
	end
	--
	...
	function Library:KeybindList()
		...
		KeyTitle.FontFace = Library.Font
		...
		function KeyList:NewKey(Name, Key, Mode)
			...
			NewValue.FontFace = Library.Font
	...
	function Library:Window(Options)
		...
		DragButton.FontFace = Library.Font
	...
	--
	function Pages:Icon(Properties)
		if not Properties then
			Properties = {}
		end
		--
	...
	function Sections:Toggle(Properties)
		...
		function Toggle:Keybind(Properties)
			...
			local ListValue = Library.Keybinds:NewKey(tostring(Keybind.State):gsub("Enum.KeyCode.", ""), Title.Text, Keybind.Mode)
			--
			Outline.Name = "Outline"
			...
			--
			local HoldLoop; 
			Library:Connection(UserInputService.InputBegan, function(inp, Typing)
				...
				if (inp.KeyCode == Key or inp.UserInputType == Key) and not Keybind.Binding then
					if Keybind.Mode == "Hold" then
						...
						HoldLoop = Library:Connection(RunService.RenderStepped, function()
			...
			--
			Library:Connection(UserInputService.InputEnded, function(inp, Typing)
				if Typing then return end 
				if Keybind.Mode == "Hold" then
	...
	function Sections:Slider(Properties)
		...
		NewSlider.FontFace = Library.Font
	...
	function Sections:Keybind(Properties)
		...
		local Toggle = Instance.new('TextButton', ModeInline)
		local Always = Instance.new('TextButton', ModeInline)
		local ListValue = Library.Keybinds:NewKey(tostring(Keybind.State):gsub("Enum.KeyCode.", ""), Keybind.Name, Keybind.Mode)
		--
		NewKey.Name = "NewKey"
		...
		--
		Library:Connection(UserInputService.InputBegan, function(inp, Chatting)
		...
		--
		Library:Connection(UserInputService.InputEnded, function(inp, Chatting)
			if not Chatting then 
		...
		--
		Library:Connection(UserInputService.InputBegan, function(Input, Chatting)
			if ModeOutline.Visible and Input.UserInputType == Enum.UserInputType.MouseButton1 then
				if not Library:IsMouseOverFrame(ModeOutline) then
					ModeOutline.Visible = false
				end
